...
| 综合任务     | 新人入职、fork 贡献、发布与回滚、多人并发、遗留仓库故障、自由实验区                                                                                            | 入职交接、fork 贡献、发布补丁与共享历史恢复、并发整合及草稿保护、遗留仓库恢复、自由实验均已验证              |
...
新增入职交接、发布回滚、多人并发、遗留仓库排错四门综合课程，以及不设固定判题的自由实验区。合计 62 个单元：61 门判题课程、122 个引导/挑战场景，加上两个独立的自由实验工作区。

- `test:capstone` 108 项断言通过。入职任务要求接续原交接提交、正确功能作者、分支跟踪、忽略个人笔记并交付远端；使用正确作者的空提交不能掩盖功能提交的错误作者。
- 发布任务要求恢复安全模式且保留功能与错误发布提交，原发布标签不能改写，新补丁标签必须有说明并交付，个人草稿不能混入发布历史。轻量标签、未推送的标签修复、丢失草稿或仍有暂存草稿均不能通过。
- 并发任务产生实际 rebase 冲突，验证 abort/continue、线性整合、普通 push 拒绝改写、租约更新，以及恢复已跟踪草稿和未跟踪笔记。遗留排错要求恢复原始丢失提交，不能用新提交代替；暂存区和工作区两份草稿都必须保留。
- 自由实验区包含 origin、upstream 和队友仓库，可以提交、推送、制造冲突、删除自己的 Git 元数据后重新初始化。它显示状态但不生成完成记录，恢复和确认重置均已验证。
- 五项新增桌面流程通过；另用真实 nano 完成交互式 rebase 清单修改、保存和退出，并通过仓库状态判题。克隆后编辑器优先选中当前任务文件，避免自动落到新生成的 `.gitignore`。
- 当前完整真实 Git 回归共 1257 项断言通过，另有 6 项 Node 测试、6 类离线工具拒绝检查、6 次 HTTP 服务生命周期、隔离/PTY 与 0.1.0 数据保留升级测试通过。依赖审计为 0 个已知漏洞。生产构建通过，仍有既有的大 JS 包提示。

覆盖矩阵中的课程均已实现并取得相应运行证据。最终安装包与默认分支的同步验证继续进行；不能将有限课程理解为穷尽全部 Git 扩展、线上平台和企业认证配置。

## 0.2.0-preview.3 交付验收

本轮课程覆盖和交付边界已逐项核对，见 [系统扩展验收](completion-audit.md)。发布版为 62 个单元：61 门判题课程（122 个场景）与两个自由实验工作区。打包程序的 26 项桌面测试全部通过，安装器解出的程序也通过空目录首次初始化及续练验证；下载、升级说明和校验值见 [preview.3 发布记录](release-0.2.0-preview.3.md)。

前文各批次的“未发布/待实现”是当时状态。当前矩阵的核心与综合工作流均有对应实现和真实运行证据；平台特有配置等边界仍按验收记录明确列出。
